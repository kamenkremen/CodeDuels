...
async def register(message: types.Message):
    ...
    try:
        ...
        await message.reply(
            "Ваш хэндл уже зарегистрирован либо данный аккаунт телеграмма уже зарегистрирован.",
            reply_markup=buttons,
        )
        return
...
def find_tests(problem):
    ...
    top1 = json.loads(
        requests.get(
            f"https://codeforces.com/api/contest.standings?contestId={cntst}&showUnofficial=true"
        ).text
    )["result"]["rows"][0]["party"]["members"]
    for hand in top1:
        handle = hand["handle"]
        if handle == None:
            continue
        res = json.loads(
            requests.get(
                f"https://codeforces.com/api/contest.status?contestId={cntst}&handle={handle}"
            ).text
        )
        res = res["result"]
...
def find_problem(middle_rating, problems, solved1, solved2, already_used=[], adj_rat=0):
    ...
    while not link:
        for i in problems:
            ...
            if (
                contest != 1
                and abs(int(i["rating"]) - middle_rating - adj_rat) <= spread
                and (i["contestId"], i["index"]) not in solved1
                and (i["contestId"], i["index"]) not in solved2
            ):
...
async def get_rating(message: types.Message):
    user_id = message["chat"]["id"]
    try:
        res = cur.execute(f"SELECT rating FROM users WHERE id == {user_id}").fetchone()[
            0
        ]
        await message.reply(f"Ваш рейтинг - {res}.", reply_markup=buttons)
    except Exception:
        await message.reply(
            "Сначала вам нужно зарегистрироваться. Напишите /register {ваш хэндл с codeforces}",
            reply_markup=buttons,
        )
        return
...
async def find_duel(message: types.Message):
    ...
    us_handle = cur.execute(
        f"SELECT handle FROM users WHERE id == {user_id}"
    ).fetchone()[0]
    await message.reply(f"Вашим соперником будет {op}.", reply_markup=buttons)
    await asyncio.sleep(1)
    rating1 = cur.execute(
        f"SELECT rating FROM users WHERE id == {opponent}"
    ).fetchone()[0]
    middle_rating = (rating1 + rating2) / 2
    spread = 0
    problems = json.loads(
        requests.get("https://codeforces.com/api/problemset.problems").text
    )["result"]["problems"]
    problems1, problems2 = json.loads(
        requests.get(f"https://codeforces.com/api/user.status?handle={op}").text
    ), json.loads(
        requests.get(f"https://codeforces.com/api/user.status?handle={us_handle}").text
    )
    ...
    await asyncio.sleep(1)
    solved_problems = [
        {us_handle: (-1, -1), op: (-1, -1)} for i in range(number_of_problems)
    ]
    status1 = json.loads(
        requests.get(
            f"https://codeforces.com/api/user.status?handle={us_handle}&count=1000"
        ).text
    )["result"]
    status2 = json.loads(
        requests.get(
            f"https://codeforces.com/api/user.status?handle={op}&count=1000"
        ).text
    )["result"]
    for solved in status1:
        for i in range(len(solved_problems)):
            prb = problems_for_duel[i]
            if (solved["problem"]["contestId"], solved["problem"]["index"]) == prb[
                1
            ] and int(solved["passedTestCount"]) > solved_problems[i][us_handle][1]:
                solved_problems[i][us_handle] = (
                    int(solved["creationTimeSeconds"]),
                    int(solved["passedTestCount"]),
                )
    for solved in status2:
        for i in range(len(solved_problems)):
            ...
            if (solved["problem"]["contestId"], solved["problem"]["index"]) == prb[
                1
            ] and int(solved["passedTestCount"]) > abs(solved_problems[i][op][1]):
                solved_problems[i][op] = (
                    int(solved["creationTimeSeconds"]),
                    int(solved["passedTestCount"]),
                )
    result = 0
    for solv in range(len(solved_problems)):
        if (solved_problems[solv][us_handle][1]) != 0:
            result += (
                (problems_for_duel[solv][2] / problems_for_duel[0][2])
                * problems_for_duel[solv][-1]
                / solved_problems[solv][us_handle][1]
                * 100
            )
